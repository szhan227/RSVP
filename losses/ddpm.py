...
class DDPM(nn.Module):
    ...
    def model_predictions_moso(self, x, mask, context, t, x_self_cond = None, clip_x_start = False):
        model_output = self.model(x, mask, t, context, None)
    ...
    @torch.no_grad()
    def ddim_sample_moso(self, x, mask, context, clip_denoised = True):
        batch, device, total_timesteps, sampling_timesteps, eta = x.shape[0], self.betas.device, self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            time_cond = torch.full((batch,), time, device=device, dtype=torch.long)
            self_cond = None
            pred_noise, x_start, *_ = self.model_predictions_moso(img, mask, context, time_cond, self_cond, clip_x_start = clip_denoised)
    ...
    def sample_moso(self, x, mask, context, return_intermediates=False):
        ...
        intermediates = [img]
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc = 'sampling loop time step'):
            img = self.p_sample_moso(img, mask, torch.full((b,), i, device=device, dtype=torch.long), 
    ...
    def p_losses_moso(self, x_start, cond, t, context, noise=None):
        ...
        model_out = self.model(x_noisy, cond[:, 0, ...], t, context)
        ...
        model_out[cond == 1] = target[cond == 1]
