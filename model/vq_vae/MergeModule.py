...
class MergeModule(nn.Module):
    def __init__(self, d_model, ds_diff_cm):
        ...
        self._bg_ds_1 = nn.Conv2d(d_model, d_model, kernel_size=3, stride=2, padding=1)
        self._bg_ds_2 = nn.Conv2d(d_model, d_model, kernel_size=3, stride=2, padding=1)
        self._id_ds_1 = nn.Conv2d(d_model, d_model, kernel_size=3, stride=2, padding=1)
        self._id_ds_2 = nn.Conv2d(d_model, d_model, kernel_size=3, stride=2, padding=1)
    ...
    def forward(self, feat_bg, feat_id, feat_m):
        ...
        feat_bg_l1 = rearrange(feat_bg, 'B T C H W->(B T) C H W')
        feat_bg_l2 = self._bg_ds_1(feat_bg_l1)
        feat_bg_l3 = self._bg_ds_2(feat_bg_l2)

        feat_id_l1 = rearrange(feat_id, 'B T C H W->(B T) C H W')
        feat_id_l2 = self._id_ds_1(feat_id_l1)
        feat_id_l3 = self._id_ds_2(feat_id_l2)

        # get multi-scale weight
        weight_l3 = self._fc_l3(torch.cat([feat_m_l3, feat_bg_l3, feat_id_l3], dim=1))
        ...
        weight = torch.sigmoid(self._fc(weight_l1))
        feat_out = weight * feat_bg_l1 + (1 - weight) * feat_id_l1
        feat_out = rearrange(feat_out, '(B T) C H W->B T C H W', T=T)
        return feat_out
