...
class UCFClassDataset(Dataset):
    def __init__(self, tokens_dir):
        ...
        self.items = []
        for cvideo in sorted(self.videos):
    ...
    def __getitem__(self, index):
        ...
        ctoken = np.load(os.path.join(self.tokens_dir, item[0], item[1], item[2]),
                         allow_pickle=True)
...
def get_dataloader(opt):
    ...
    trainset_sampler = InfiniteSampler(dataset=trainset)
...
def get_dataset(dataset_opt):
    if dataset_opt.cname == 'UCFClassDataset':
        trainset = UCFClassDataset(tokens_dir=dataset_opt.train.tokens_dir)
        validset = UCFClassDataset(tokens_dir=dataset_opt.valid.tokens_dir)
    else:
