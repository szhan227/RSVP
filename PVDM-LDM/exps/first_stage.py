...
def first_stage(rank, args):
    device = torch.device('cuda', rank) # This is an integer that indicates which GPU to use.

    temp_dir = './'
    if args.n_gpus > 1:
        init_file = os.path.abspath(os.path.join(temp_dir, '.torch_distributed_init'))
        if os.name == 'nt': # On Windows, it will return 'nt'. On macOS, it will return 'posix'. On Linux, it will return 'linux'.
    ...
    init_multiprocessing(rank=rank, sync_device=sync_device) # collecting statistics across multiple processes
    ...
    if args.n_gpus > 1:
        '''
        DistributedDataParallel (DDP) is a PyTorch API that allows you to 
        train your models on multiple GPUs.
        '''
